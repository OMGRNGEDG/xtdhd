...
Page({
  ...
  getOperateChart(token) {
    // 请求echarts
    app.api.GET({}, config.Project + token + '/storeoperateshart').then(res => {
      if (res.result_text == "success" && res.result == 3) {
        res.data.forEach(ele => {
          XList.push(ele.month + '月'+ele.day+'日');
  ...
  getOperate(options) {
    ...
    app.api.GET(_data, config.Project + options + '/storeoperate').then(res => {
      if (res.result_text == "success" && res.result == 3) {
  ...
  gotoOnce(e) {
    ...
    wx.navigateTo({
      url: '/pages/commonList/index?id=' + _id + '&&path=' + 'pages/storeIncome/index' + '&name=' + this.data.userName
    });
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
